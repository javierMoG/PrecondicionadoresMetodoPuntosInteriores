function [x, lambda, mu, z, norma, cinter, mediaIterK, totIterK] = IntPointPrecondA(Q,A,F,d,b,c)
...
% Precondicionador de Tim Rees y Chen Greif
...
while(norma > tol && cinter < maxiter  && condK > eps)
    ...
    LP = [L11  zeros(n,m); zeros(m,n) L22];
    % Sistema Lineal a resolver
    [y,~,~,mIter] = minres(K,ld,1.e-05,100,LP,LP');
    kIter= [kIter mIter];
    %-------------------------------------------------------
